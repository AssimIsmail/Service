...
const Support_plaine = () => {
  ...
  const fetchSupport = async () => {
    try {
      const response = await axios.get(
        `http://127.0.0.1:8000/api/supports/${id}`
      );
  ...
  const fetchReponses = async () => {
    try {
      const response = await axios.get(
        `http://127.0.0.1:8000/api/reponses?support_id=${id}`
      );
          ...
          {support && (
            <div>
              <p className="text-lg font-bold text-center">
                Référence: {support.id}
              </p>
              <h2 className="text-xl font-bold text-center">
                Réclamation: {support.raison}
              </h2>
                  ...
                  {!isCurrentUser && reponseUser && (
                    <div className="flex items-center mb-2">
                      <div
                        ...
                        style={{
                          backgroundImage: `url('${reponseUser.image}')`,
                        }}
                      ></div>
                      <p>
                        <strong>{reponseUser.lastname}</strong>{" "}
                        {reponseUser.username}
                      </p>
                    </div>
                  )}
                  {isCurrentUser && reponseUser && (
