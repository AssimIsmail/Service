...
const ServicePricing = ({ basic, standard, utilisateur_id, service_id, premium, selectedPlan, handlePlanSelect, handleContinue }) => {
  const { user } = useContext(UserContext);
  const [serviceDetails, setServiceDetails] = useState(null);
  const connectedUserId = user ? user.id : 3000000000000000; 

  useEffect(() => {
    const fetchServiceDetails = async () => {
      try {
        const response = await axios.get(`http://127.0.0.1:8000/api/services/${service_id}`);
      ...
      <div className="border mt-4 min-h-48 h-auto p-4">
        {selectedPlan === "basic" && basic && basic[0] && (
          <div className="border p-4 mb-4">
            ...
            {(connectedUserId && serviceDetails && serviceDetails.utilisateur_id !== connectedUserId) && (
              <button
