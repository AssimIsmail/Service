...
const Comments = ({ commentaires, utilisateur_id, service_id, users, user, handleSubmitComment, fetchData }) => {
  ...
  const fetchDemandes = async () => {
    try {
      const response = await axios.get(`http://127.0.0.1:8000/api/demandes?utilisateur_id=${user.id}&&dev=${utilisateur_id}`);
  ...
  const demandeTerminée = demandes && demandes.some(demande => demande.status === "Terminer");
  ...
  const handleCommentSubmit = async () => {
    setLoading(true); 
    await handleSubmitComment(newComment, stars); 
    await fetchData(); 
    setLoading(false);
  ...
  const handleDeleteComment = async (commentId) => {
    try {
      setLoading(true);
      await axios.delete(`http://127.0.0.1:8000/api/commentaires/${commentId}`);
      await fetchData(); // Supposant que fetchData() recharge les commentaires mis à jour
      setLoading(false);
    } catch (error) {
          ...
          {commentaires && commentaires.length > 0 ? (
            <div className="flex flex-col space-y-4">
              {(showAllComments ? commentaires : commentaires.slice(0, 3)).map(
                (commentaire, index) => {
                  const userFound = users && users.find((user) => user.id === commentaire.utilisateur_id);
                  return (
